...
function ExtentBar({
  ...
  return (
    <div className="mb-2">
      ...
      <div className="h-5 rounded relative bg-gray-700">
        <div
          className="absolute top-0 h-full rounded"
          style={{
            left: `${bar.startPct}%`,
            width: `${Math.max(1, bar.endPct - bar.startPct)}%`,
            backgroundColor: color,
          }}
        />
        <span className="absolute left-1 top-0 text-[9px] font-bold text-white/70 leading-5">
          {labels[0]}
        </span>
        <span className="absolute left-1/2 -translate-x-1/2 top-0 text-[9px] font-bold text-white/70 leading-5">
          {labels[1]}
        </span>
        <span className="absolute right-1 top-0 text-[9px] font-bold text-white/70 leading-5">
          {labels[2]}
        </span>
      </div>
...
export default function GnuInfographicsPage() {
  const [entries, setEntries] = useState<GnuInfographicEntry[]>([]);
  ...
  const sorted = [...entries].sort((a, b) => {
    if (sortBy === "riderStart") {
      const aS = a.analysis?.riderLevel.startPct ?? 50;
      const bS = b.analysis?.riderLevel.startPct ?? 50;
      return aS - bS;
    }
    if (sortBy === "riderEnd") {
      const aE = a.analysis?.riderLevel.endPct ?? 50;
      const bE = b.analysis?.riderLevel.endPct ?? 50;
      return aE - bE;
    }
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 max-w-[1600px] mx-auto">
      <header className="mb-6">
        <h1 className="text-2xl font-bold mb-1">GNU Infographic Audit</h1>
        <p className="text-gray-400 text-sm">
          GNU infographics use a lens/almond-shaped gradient above a black scale
          border. The colored region shows the extent bar for each property.
        </p>
      </header>
                      ...
                      {entry.analysis ? (
                        <div className="w-56">
                          <ExtentBar
                            ...
                            label="TERRAIN"
                            labels={["PARK", "ALL MTN", "BACKCOUNTRY"]}
                            color="#f59e0b"
                          />
                          <ExtentBar
