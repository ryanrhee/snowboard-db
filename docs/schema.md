...
One row per unique snowboard product. Primary key is `board_key`, formatted as `brand|model|gender` (e.g. `burton|custom|unisex`, `capita|paradise|womens`, `burton|after school special|kids`).

```sql
CREATE TABLE boards (
  board_key         TEXT PRIMARY KEY,  -- "brand|model|gender" lowercase
  brand             TEXT NOT NULL,
  model             TEXT NOT NULL,
  gender            TEXT NOT NULL DEFAULT 'unisex',  -- unisex, womens, kids
  year              INTEGER,
...
CREATE TABLE listings (
  ...
  retailer           TEXT NOT NULL,     -- tactics, evo, backcountry, rei
...
- **`board_key`**: Generated by `specKey()` in `src/lib/db.ts`. Uses `BrandIdentifier` for canonical brand name and `getStrategy(manufacturer).identify()` for model normalization. Format: `brand|model|gender` where gender is `unisex`, `womens`, or `kids`.
  - Example: `"Burton"` + `"Custom Snowboard - 2026"` → `"burton|custom|unisex"`
  - Example: `"CAPiTA"` + `"Paradise - Women's"` → `"capita|paradise|womens"`
  - The strategy strips manufacturer-specific noise (profile codes, rider names, etc.) while preserving meaningful model identity.
  - For kids/youth boards, a leading `"kids "` prefix is stripped from the model (e.g. `"burton|custom smalls|kids"` not `"burton|kids custom smalls|kids"`).
...
- **`Board`** — one row from `boards` table, includes `gender` field
- **`Listing`** — one row from `listings` table, includes `condition`, `gender`, `comboContents`
...
- **`ScrapedBoard`** — unified scraper output (one per board model per source, with listings array for retailers). Uses `brandId: BrandIdentifier` instead of a plain brand string.
- **`ScraperModule`** — unified scraper interface (name, sourceType, baseUrl, scrape())
- **`RawBoard`** — internal intermediate type within retailer scrapers (one per size/listing). `brand` field typed as `BrandIdentifier` (retailers create instances at scrape time).

### Strategy Types (`src/lib/strategies/`)

- **`BrandIdentifier`** — immutable identifier: `raw` → `cleaned` → `canonical` → `manufacturer`. Drives strategy dispatch.
- **`BoardSignal`** — immutable input to strategy: raw model, brand, manufacturer, source, sourceUrl, optional profile/gender
- **`BoardIdentity`** — strategy output: normalized `model` + `profileVariant` (or null)
- **`BoardIdentificationStrategy`** — interface with `identify(signal): BoardIdentity`
...
Retailers (Evo, Tactics, ...)    Manufacturers (Burton,
         │                        Lib Tech, CAPiTA, ...)
         │                                │
    RawBoard[] with                 ManufacturerSpec[]
    BrandIdentifier                       │
         │                                │
    adaptRetailerOutput()         adaptManufacturerOutput()
    (strategy.identify()           (wraps as ScrapedBoard)
     for grouping)                        │
         │                                │
         ▼                                ▼
    ScrapedBoard[]               ScrapedBoard[]
         ...
         └────────────┬───────────────────┘
               ...
               ┌──────┴──────┐
               │  For each board:
               │  BrandIdentifier.manufacturer
               │       → getStrategy()
               │       → strategy.identify(signal)
               │       → { model, profileVariant }
               │  Group by specKey(brand|model|gender)
               │  Split if multiple profileVariants
               └──────┬──────┘
                      │
              board keys: BoardGroup[]
                      │
                      ▼
